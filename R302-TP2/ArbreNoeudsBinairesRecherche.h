...
int ArbreNoeudBinaireRecherche<TypeInfo>::getHauteurRecWorker(NoeudBinaire<TypeInfo>* ptrRac) const {
    ...
    int hauteurGauche = getHauteurRecWorker(ptrRac->getRefPtrFilsGauche());
    int hauteurDroite = getHauteurRecWorker(ptrRac->getPtrFilsDroit());

    return 1 + std::max(hauteurGauche, hauteurDroite);
}
...
TypeInfo ArbreNoeudBinaireRecherche<TypeInfo>::getAncetreCommunLePlusBasRecWorker(const NoeudBinaire<TypeInfo>* ptrRac,
    ...
    if (ptrRac->getPtrFilsGauche(val1)==nullptr) {
        ptrRac->getPtrFilsDroit(val1);
        ptrRac->getPtrFilsGauche(val2);
    }
    return 0;
}
