...
void ListeTrieeChainee<TypeInfo>::supprimeToutesDuplications() {
    Cellule<TypeInfo>* current = ptrTete;
    while (current && current->getSuivante()) {
        if (current->getInfo() == current->getSuivante()->getInfo()) {
            Cellule<TypeInfo>* duplicate = current->getSuivante();
            current->setSuivante(duplicate->getSuivante());
            delete duplicate;
            nbCellules--;
        } else {
 ...
 * @return Position d'uneInfo si présentais –position qu'uneInfo aurait dû occuper si absente
 */
template<class TypeInfo>
int ListeTrieeChainee<TypeInfo>::getPositIter(const TypeInfo& uneInfo) const {
    ...
    Cellule<TypeInfo>* current = ptrTete;
    while (current) {
        if (current->getInfo() == uneInfo) {
            return position;
        } else if (current->getInfo() > uneInfo) {
            return -position;
        }
        current = current->getSuivante();
        position++;
...
void ListeTrieeChainee<TypeInfo>::insereRecWorker(Cellule<TypeInfo>*& ptrCetteListe, const TypeInfo& nouvelleInfo) {
    if (ptrCetteListe == nullptr || nouvelleInfo < ptrCetteListe->getInfo()) {
        Cellule<TypeInfo>* nouvelleCellule = new Cellule<TypeInfo>(nouvelleInfo, ptrCetteListe);
        ptrCetteListe = nouvelleCellule;
        nbCellules++;
    } else {
...
/**
 * Suppression de la première occurrence d'uneInfo si présente et retour true, si absente retour false
 * @tparam TypeInfo
...
bool ListeTrieeChainee<TypeInfo>::supprimePremOccInfoRecWorker(Cellule<TypeInfo>*& ptrCetteListe, const TypeInfo& uneInfo) {
    ...
    if (ptrCetteListe->getInfo() == uneInfo) {
...
bool ListeTrieeChainee<TypeInfo>::supprimePremOccInfoIter(const TypeInfo& uneInfo) {
    Cellule<TypeInfo>* current = ptrTete;
    Cellule<TypeInfo>* previous = nullptr;
    while (current) {
        if (current->getInfo() == uneInfo) {
            if (previous) {
                previous->setSuivante(current->getSuivante());
            } else {
                ptrTete = current->getSuivante();
            }
            delete current;
            nbCellules--;
            return true;
...
ListeTrieeChainee<TypeInfo>* ListeTrieeChainee<TypeInfo>::insersectionAvec(const ListeTrieeChainee<TypeInfo>* ensembleB) const {
    ListeTrieeChainee<TypeInfo>* result = new ListeTrieeChainee<TypeInfo>();
    Cellule<TypeInfo>* a = this->ptrTete;
    Cellule<TypeInfo>* b = ensembleB->ptrTete;

    while (a && b) {
        if (a->getInfo() < b->getInfo()) {
            a = a->getSuivante();
        } else if (a->getInfo() > b->getInfo()) {
            b = b->getSuivante();
        } else {
            result->insereRec(a->getInfo());
            a = a->getSuivante();
            b = b->getSuivante();
        }
    }
...
ListeTrieeChainee<TypeInfo>* ListeTrieeChainee<TypeInfo>::unionAvec(const ListeTrieeChainee<TypeInfo>* ensembleB) const {
    ...
    while (a || b) {
        if (!b || (a && a->getInfo() < b->getInfo())) {
            result->insereRec(a->getInfo());
            a = a->getSuivante();
        } else if (!a || (b && b->getInfo() < a->getInfo())) {
            result->insereRec(b->getInfo());
            b = b->getSuivante();
        } else {
            result->insereRec(a->getInfo());
            a = a->getSuivante();
