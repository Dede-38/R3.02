...
ListeChainee<TypeInfo>::ListeChainee(const ListeChainee<TypeInfo> &uneListeChainee) : nbCellules(uneListeChainee.nbCellules) {
    ...
    if (ptrListeOriginale == nullptr) {
        ...
        while (ptrListeOriginale != nullptr) {
            ...
            // mettre à jour le pointeur sur la dernière cellule de la nouvelle liste
...
bool ListeChainee<TypeInfo>::insereAtPositIter(const int nouvellePosition, const TypeInfo &nouvelleInfo) {
    ...
    if (nouvellePosition >= 1 && nouvellePosition <= nbCellules+1 ) {
        if (nouvellePosition ==1) {
            insereTeteWorker(ptrTete, nouvelleInfo);
                ...
                Cellule<TypeInfo> *ptrCour = ptrTete;
                for (int i = 0; i < nouvellePosition-2; i++) {
                    ptrCour = ptrCour->getSuivante();
                }
                insereTeteWorker(ptrCour->getRefSuivante(), nouvelleInfo);
            }
            return true;
        }
    else {return false;}
    ...
    //Méthode avancée
    // Cellule<TypeInfo> **ppListe = &ptrTete;
    // for (int i = 0; i < nouvellePosition; i++) {
    //     ppListe = &(*ppListe)->getRefSuivante();
    // }
    // insereTeteWorker(*ppListe, nouvelleInfo);
    // return true;
    }
...
bool ListeChainee<TypeInfo>::supprimeAtPositIter(int position) {
     ...
     * À COMPLETER
     */
    if (ptrTete && position <= nbCellules ) {
    ...
    if (position == 1) {
        ...
        supprimeTeteWorker(ptrCour->getRefSuivante());
    }
    //Méthode avancée
    // if (ptrTete && position <= nbCellules ) {
    //
    ...
    //         ppListe = &(*ppListe)->getSuivante();
    // }
    // supprimeTeteWorker(*ppListe);
...
bool ListeChainee<TypeInfo>::estInfoPresentRecWorker(Cellule<TypeInfo> *ptrCetteListe, TypeInfo &infoCible) const {
     ...
     * À COMPLETER
     */
    if (!ptrCetteListe) {return false;}
    else if (ptrCetteListe->getInfo() == infoCible) {
        return true;
    }
    else{return estInfoPresentRecWorker(ptrCetteListe->getSuivante(), infoCible);}
}
...
// On est donc certain de trouver pas besoin de considérer une liste videRec !
//
// Noter que le premier paramètre est une référence sur un pointeur qui n'est pas const ainsi un paramètre résultat
template<class TypeInfo>
void ListeChainee<TypeInfo>::supprimeAtPositRecWorker(Cellule<TypeInfo>*& ptrCetteListe, int position) {
    ...
    if (ptrCetteListe != nullptr && position==1) {
        Cellule<TypeInfo> *ptrSuppr = ptrCetteListe;
...
void ListeChainee<TypeInfo>::afficheGDWorker(const Cellule<TypeInfo> *ptrListe) const {
    ...
    // sinon rien à faire, donc on n'en fait rien !!!
}
